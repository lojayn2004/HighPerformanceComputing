...
int main(int argc, char** argv) {
    ...
    if(rank == 0) {
       ...
       // Root counts primes in  its range 
       ...
       // Root sends the start & end to each other process so each process could calculate its range
       for(int i = 1;i < size;i++) {
          MPI_Send(&st, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
          MPI_Send(&e, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
       }

       // After each process calculates its sum the root recives that sum & adds it to the global sum 
       for(int i = 1;i < size;i++) {
          int part_sum = 0;
       ...
       printf("Total Time needed %f \n", end - start);
          ...
          // Each process recieves the start & the end from the root to calculate the range 
          // over which it will count the primes 
